...
- **Images/photos — don't disturb what's already placed; adding is fine when we're designing.** When actively designing a section together, you MAY add and choose photos for that section. What's prohibited is changing, replacing, or overwriting an image already on the site when we're *not* working on that part. Shared image files are reused across pages, so editing one changes every usage — e.g. overwriting `/public/nhg-book.jpg` (the real *Neurosis and Human Growth* cover) silently changed BOTH the register page and NHG "Start Here." For a new image need, add a NEW file — never repurpose/overwrite an existing shared one. When restoring, prefer the original from git history.
