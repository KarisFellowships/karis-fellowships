    ...
    <div className="min-h-screen bg-slate-dark pt-20">
      ...
      <section className="relative overflow-hidden px-6 py-14">
        <div className="absolute inset-0">
          <Image src="/forest-light.jpg" alt="Light breaking through a forest canopy" fill className="object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-slate-dark/85 via-slate-dark/70 to-slate-dark/50" />
        </div>
        <div className="relative mx-auto max-w-6xl">
          <p className="text-sm font-medium text-coral/80">NHG Book Study</p>
          <h1 className="mt-1 text-3xl font-bold text-white">Neurosis and Human Growth</h1>
          <p className="mt-2 text-white/50">Your foundational study for the Karis journey.</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2">
              {activeWeek ? (
                <div className="overflow-hidden rounded-2xl bg-white/5 ring-1 ring-white/10 p-8">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <span className="inline-block rounded-full bg-coral px-3 py-1 text-xs font-bold text-white">
                        This Week
                      </span>
                    ...
                    {currentGuide && (
                      <div className="flex flex-col gap-2">
                        <a
                          href={currentGuide.guide}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 rounded-lg bg-coral/15 px-4 py-2.5 text-sm font-semibold text-coral transition-all hover:bg-coral/25"
                        >
                          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                          Reading Guide
                        </a>
                        <a
                          href={currentGuide.intro}
                          target="_blank"
                          rel="noopener noreferrer"
            ...
            <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-6">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="mt-0.5 text-lg font-bold text-coral">(701) 801-1220</p>
                </div>
                <div className="border-t border-white/10 pt-3">
                  <p className="text-xs text-white/35 uppercase tracking-wide">Format</p>
                  <p className="mt-0.5 text-sm text-white/60">Conference call-based book study</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <a
              href="/docs/nhg/kf-intro/NHG-Book-Study-Syllabus-2025 (1).pdf"
              target="_blank"
              rel="noopener noreferrer"
              className="group rounded-2xl bg-white/5 ring-1 ring-white/10 p-5 transition-all hover:bg-white/10 hover:-translate-y-0.5"
            >
              <h3 className="font-bold text-white group-hover:text-coral transition-colors">NHG Book Study Syllabus</h3>
              <p className="mt-1 text-xs text-white/40">Study overview and plan</p>
            </a>
            <a
              href="/docs/nhg/review-study-questions-v2a.pdf"
              target="_blank"
              rel="noopener noreferrer"
              className="group rounded-2xl bg-white/5 ring-1 ring-white/10 p-5 transition-all hover:bg-white/10 hover:-translate-y-0.5"
            >
              <h3 className="font-bold text-white group-hover:text-coral transition-colors">Review Study Questions</h3>
            ...
            <Link
              ...
              className="group rounded-2xl bg-white/5 ring-1 ring-white/10 p-5 transition-all hover:bg-white/10 hover:-translate-y-0.5"
            >
              <h3 className="font-bold text-white group-hover:text-coral transition-colors">Recordings</h3>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-sm font-bold ${
                        isCurrent ? "bg-coral text-white" : "bg-coral/20 text-coral"
                      }`}>
                        {week.weekNumber}
                      </span>
                      <div>
                        <h3 className={`text-sm font-semibold ${isCurrent ? "text-coral" : "text-white"}`}>
                          {week.label}
                        </h3>
                        <p className="text-xs text-white/35">{week.dateRange}</p>
                        {isCurrent && <span className="text-xs font-medium text-coral">This Week</span>}
                      </div>
                    </div>
                    {guide && (
                      <div className="mt-3 flex gap-2">
                        <a href={guide.guide} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-coral/70 hover:text-coral transition-colors">
                          Reading Guide
                        </a>
                        <span className="text-white/20">|</span>
                        <a href={guide.intro} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-amber/70 hover:text-amber transition-colors">
                          Introduction
                        </a>
                      </div>
                    )}
              ...
              <div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {kfIntroDocuments.map(({ label, href }) => (
                    <a key={href} href={href} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-teal-light/70 hover:text-teal-light transition-colors">
                      {label}
                    </a>
                  ))}
                </div>
          ...
          {/* All Reading Guides */}
          <div className="mt-12">
            <h2 className="text-xl font-bold text-white">All Reading Guides</h2>
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {Object.entries(readingGuides).map(([num, { label, guide, intro }]) => (
                <div key={num} className="flex items-center justify-between rounded-xl bg-white/5 ring-1 ring-white/10 p-3">
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-coral/15 text-xs font-bold text-coral">
                      {num}
                    </span>
                    <span className="text-sm text-white/70">{label}</span>
                  </div>
                  <div className="flex gap-2">
                    <a href={guide} target="_blank" rel="noopener noreferrer" className="rounded-md bg-coral/10 px-2 py-1 text-xs font-semibold text-coral hover:bg-coral/20 transition-colors">
                      Guide
                    </a>
                    <a href={intro} target="_blank" rel="noopener noreferrer" className="rounded-md bg-amber/10 px-2 py-1 text-xs font-semibold text-amber hover:bg-amber/20 transition-colors">
                      Intro
                    </a>
                  </div>
                </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  {activeWeek && facilitatorGuides[activeWeek.weekNumber] && (
                    <div className="mt-4 rounded-xl bg-violet/10 ring-1 ring-violet/20 p-5">
                      <span className="inline-block rounded-full bg-violet/20 px-3 py-1 text-xs font-bold text-violet-light">
                        This Week
                      </span>
                      ...
                      <a
                        href={facilitatorGuides[activeWeek.weekNumber].href}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-3 inline-flex items-center gap-2 rounded-lg bg-violet/20 px-4 py-2.5 text-sm font-semibold text-violet-light transition-all hover:bg-violet/30"
                      >
                        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      {facilitatorResources.map(({ label, href }) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="group flex items-center gap-3 rounded-xl bg-white/5 ring-1 ring-white/10 p-3 transition-all hover:bg-white/10 hover:-translate-y-0.5"
                        >
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-violet/15">
                            <svg className="h-4 w-4 text-violet-light" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                          </span>
                          <span className="text-sm font-medium text-white/70 group-hover:text-violet-light transition-colors">{label}</span>
                        </a>
                      ))}
                    </div>
                  </div>
                        ...
                        return (
                          ...
                          >
                            <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-xs font-bold ${
                              isCurrent ? "bg-violet text-white" : "bg-violet/15 text-violet-light"
                            }`}>
                              {num}
                            </span>
                            <span className={`text-sm font-medium transition-colors ${
                              isCurrent ? "text-violet-light" : "text-white/60 hover:text-violet-light"
                            }`}>
                              Week {num}
                              {isCurrent && <span className="ml-1 text-xs text-violet-light/70">(This Week)</span>}
                            </span>
                          </a>
                        );
                      })}
