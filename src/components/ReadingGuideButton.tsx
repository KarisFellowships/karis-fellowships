...
export default function ReadingGuideButton({ pdf, doc, variant = "icon", label = "Open Reading Guide" }: Props) {
  ...
  // The outer wrapper sits flush against the trigger (bottom-full, no margin) and
  // carries the visual gap as padding, so moving the mouse from the trigger to the
  // menu never crosses a dead zone that would close it.
  const menu = (
    <div className="absolute bottom-full left-1/2 z-30 -translate-x-1/2 pb-2">
      <div
        role="menu"
        className="w-36 overflow-hidden rounded-xl border border-white/15 bg-slate-dark/95 p-1 shadow-xl shadow-black/40 backdrop-blur-md"
      >
